...
func _on_data_entity_collision_mouse_entered():
	if not dragging and not correct:
		draggable = true
		scale = Vector2(5.1, 5.1)


func _on_data_entity_collision_mouse_exited():
	if not dragging and not correct:
		draggable = false
		scale = Vector2(5, 5)


func _on_data_entity_collision_body_entered(body):
	if body.is_in_group('data_droppable'):
		droppable = true
...
func _on_data_entity_collision_body_exited(body):
	if body.is_in_group('data_droppable'):
		droppable = false
